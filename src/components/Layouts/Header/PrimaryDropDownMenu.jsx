...
const PrimaryDropDownMenu = ({ setTogglePrimaryDropDown,setToggleMobileMenu, user }) => {
  ...
  return (
    <div className="absolute w-60 right-0 sm:right-auto sm:-left-24 ml-2 top-16 sm:top-9 bg-white shadow-2xl rounded flex-col text-sm">
      {user.role === "admin" && (
        <Link
      ...
      <Link
        onClick={() => {setTogglePrimaryDropDown(false); setToggleMobileMenu(false)}}
        className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50 rounded-t"
        to="/account"
      >
        <span className="text-primary-blue">
