...
const NewProduct = () => {
  ...
  const options = [
    { value: "Twisted Wick Colored", label: "Twisted Wick Colored" },
    { value: "cotton wicks", label: "cotton wicks" },
    { value: "special wicks", label: "special wicks" },
  ];
  ...
  const newProductSubmitHandler = async (e) => {
    ...
    if (serverResponse) {
      ...
      setDisabled(false);
      enqueueSnackbar("Product Created Successful", { variant: "success" });
    } else {
      setDisabled(false);
  ...
  return (
    <>
      ...
      <form
        ...
        className="flex flex-col lg:flex-row bg-white rounded-lg shadow p-4"
        id="mainform"
      >
        <div className="flex flex-col gap-3 m-2 lg:w-1/2">
          ...
          {/* <div>
            <label className="flex gap-2 cursor-pointer">
              <input onChange={() => setIsGroup(!isGroup)} type="checkbox" />
              <h2 className="font-medium">Display in group</h2>
            </label>
            <div className={`${!isGroup && "hidden"}`}>
              ...
              />
              <div>
                <h2 className="text-green-700 py-1">
                  {selectedOptions?.__isNew__
                    ? "Creating a new group : "
                    : "You selected :"}
                </h2>
                <p className="p-2 border rounded font-medium">
                  {selectedOptions?.label ? selectedOptions?.label : "Nothing"}
                </p>
              </div>
            </div>
          </div> */}
        ...
        <div className="flex flex-col gap-2 m-2 lg:w-1/2">
          ...
          <div className="flex justify-end items-center">
            <button
              ...
              form="mainform"
              type="submit"
              className="bg-primary-orange uppercase mx-auto w-1/3 p-3 text-white font-medium rounded shadow hover:shadow-lg cursor-pointer"
              value="Submit"
            >
              {!disabled ? (
                <span>Submit</span>
