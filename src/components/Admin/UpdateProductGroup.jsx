...
export default function UpdateProductGroup() {
  ...
  const initializeExistingGroup = async () => {
    const resp = await publicCommunication.getGroupProductsById(params.id);
    if (resp?.data) {
      console.log(resp.data);
      setGroupName(resp?.data?.group?.name);
      let temp = [];
  ...
  const handleSubmit = async () => {
    ...
    const { data } = await adminCommunication.updateGroup(body, params.id);
    if (data.success) {
      setDisabled(false);
      ...
      <div className="bg-white p-2 rounded h-[90%] mt-5">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-blue-500 border border-blue-500 hover:bg-white hover:text-blue-500 px-4 rounded text-lg font-medium text-white"
          >
            Update
          </button>
        </div>
