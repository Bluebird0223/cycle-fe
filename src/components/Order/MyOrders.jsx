...
const MyOrders = () => {
  ...
  const getUserOrders = async () => {
    try {
      ...
      if (serverResponse?.data?.success) {
        ...
        dispatch(
          newOrderFail(serverResponse?.data?.message || "Something went wrong!")
        );
      }
    } catch (error) {
  ...
  return (
    <>
      <MetaData title="My Orders | KanchanDeepJyot" />

      <main className="w-full pt-2 sm:mt-0 ">
        {/* <!-- row --> */}
        <div className="flex gap-3.5 mt-2 sm:mt-6 sm:mx-3 m-auto mb-7">
          {/* <!-- sidebar column  --> */}
          <div className="hidden sm:flex flex-col w-1/5 px-1">
            {/* <!-- nav tiles --> */}
            <div className="flex flex-col bg-white    -gray-700 rounded-sm shadow">
              {/* <!-- filters header --> */}
              <div className="flex items-center justify-between gap-5 px-4 py-2 border-b -gray-600">
                <p className="text-lg font-medium">Filters</p>
                <span
                  onClick={clearFilters}
                  className="text-red-800 font-medium text-sm uppercase cursor-pointer hover:text-blue-700"
                >
                  clear all
                </span>
              </div>
              ...
              <div className="flex flex-col py-3 text-sm">
                ...
                {/* <!-- checkboxes --> */}
                <div className="flex flex-col gap-3 px-4 mt-1 pb-3 border-b -gray-600">
                  <FormControl>
                    <RadioGroup
